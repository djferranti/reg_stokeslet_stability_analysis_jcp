function parElasticNonlinearAlgebraic(dt, tFinal, epsilonFac, N, bending)
%implements the doubly periodic elastic surface-fluid problem using either
%of the algebraic blobs, phi^A or phi^{A,3M}. 
%% geometry
NInterp = 2 * N;

if bending 
    matname = ['par ' num2str(N) 'BendingClassicEpsilon' num2str(round(epsilonFac)) 'h.mat'];
    fileID = fopen(['bendingAlgebraicL2_' num2str(N) '_Epsilon' num2str(round(epsilonFac)) '.txt'],'w');
    vidObj = VideoWriter(['bendingAlgebraic' num2str(N) '_Epsilon' num2str(epsilonFac) '.avi'], "Motion JPEG AVI");
    vidObj.Quality=100;
    open(vidObj);
else 
    matname = ['par ' num2str(N) 'TensionOnlyClassicEpsilon' num2str(round(epsilonFac)) 'h.mat'];
    fileID = fopen(['tensionOnlyAlgebraicL2_' num2str(N) '_Epsilon' num2str(round(epsilonFac)) '.txt'],'w');
    vidObj = VideoWriter(['tensionAlgebraic' num2str(N) '_Epsilon' num2str(epsilonFac) '.avi'], "Motion JPEG AVI");
    vidObj.Quality=100;
    open(vidObj);
end
...
for t = 0 : dt : tFinal %do the following at every time step
    ...
    %for the "7/2" blob, phi^A in paper (no moment conditions)
    % [U, V, W] = ...
    %     evaluatevelocity72(Fq, Fs, Fz, Xq, Xs, Xz, ...
    %     X, Y, Z, dS,L, Kx, Ky, C, epsilon, mu, NInterp); 

    %for the "13/2" blob, phi^{A,3M} in paper (3 moment conditions)
    [U, V, W] = ...
       evaluatevelocity132(Fq, Fs, Fz, Xq, Xs, Xz, ...
       X, Y, Z, dS,L, Kx, Ky, C, epsilon, mu, NInterp);

    try 
    [Usurface, Vsurface, Wsurface] = ...
        interpolatetosurface(xj, yj, zLevels, L, U, V, ...
        W, Xq, Xs, Xz, isperiodic);
    catch  
      disp('interpolation failed - likely nans')
      break 
    end
    ...
    l2U = sqrt ( sum ( sum ( (Usurface.^2 + Vsurface .^2 + ...
        Wsurface .^2) ./ (N .* N) ) ) );
    ...
    if (mod(timeStepCounter, skip) == 0)
        vidObj=plotsurface(vidObj,Xq, Xs, Xz, L, l2U, t);
    end 
...
function [U2dp, V2dp, W2dp] = ...
        evaluatevelocity72(Fq, Fs, Fz, ...
        Xq, Xs, Xz, X, Y, Z, dS, L, Kx, Ky, C, ...
        epsilon, mu, NInterp) 
...
parfor k = 1 : numberForces 
    forceK = [Fq(k), Fs(k), Fz(k)]' .* dS(k); 
    xyzK = [Xq(k), Xs(k), Xz(k)]'; 
    [U2dpK, V2dpK, W2dpK] = ...
        dpregularizedstokesletclassic(C, Kx, Ky, ...
        X, Y, Z, L, xyzK, forceK, epsilon, mu, NInterp); 
