function [U2dp, V2dp, W2dp] = dpregularizedstokesletnewalgebraic(C, Kx, Ky, ...
...
for j = 1 : numberZlevels 
    ...
    Gfour = Ghat132(C, Kx, Ky, z, epsilon, x0, y0);
    Bfour = Bhat132(C, Kx, Ky, z, epsilon, x0, y0);
    Bprimefour = dBhat132dz(C, Kx, Ky, z, epsilon, x0, y0);
    ...
    G0 = (1/16).*pi.^(-1).*(epsilon.^2+z.^2).^(-7/2).*(12.*epsilon.^6.* ...
        z.^2+17.*epsilon.^4.*z.^4+14.*epsilon.^2.*z.^6+4.*z.^8);
